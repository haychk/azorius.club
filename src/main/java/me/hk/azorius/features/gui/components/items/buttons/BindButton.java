...
public class BindButton
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        if (this.isListening) {
            OyVey.textManager.drawStringWithShadow("Press a Key...", this.x + 2.3f, this.y - 1.7f - (float) me.alpha432.oyvey.features.gui.AzoriusGui.getClickGui().getTextOffset(), -1);
        } else {
            OyVey.textManager.drawStringWithShadow(this.setting.getName() + " " + ChatFormatting.GRAY + this.setting.getValue().toString().toUpperCase(), this.x + 2.3f, this.y - 1.7f - (float) me.alpha432.oyvey.features.gui.AzoriusGui.getClickGui().getTextOffset(), this.getState() ? -1 : -5592406);
        }
    }
